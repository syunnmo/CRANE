...
class CustomLSTMwithLE(nn.Module):
    ...
    def forward(self, x, prev_h, prev_c, curr_le):
        ...
        # ht = torch.tanh(self.W_h(torch.cat((h_t_tilde, curr_le), dim=1)))
...
class KnowledgestateAcquisition(nn.Module):
    ...
    def forward(self, interactions):
        ...
        # gain0 = utils.varible(torch.nn.Parameter(torch.randn(batch_size, self.hidden_dim), requires_grad=True), self.gpu)
        ...
        for i in range(len):
            ...
            if i % self.interval == 0 and i != 0:
                ...
                # gain = torch.sigmoid(self.W_ga(torch.mul(prev_prev_h, prev_h)) / self.interval)
                # gain = torch.sigmoid(torch.sum(prev_prev_h * prev_h, dim=1) / self.interval)
                gain = (1 - F.cosine_similarity(prev_prev_h, prev_h, dim=1)) / (self.interval)

                # curr_le = torch.mul((1 + gain - prev_gain), prev_le)

                curr_le = (1 + (gain - prev_gain)).unsqueeze(1) * prev_le
                # curr_le = (gain - prev_gain).unsqueeze(1) * prev_le
                # curr_le = normal_gain.unsqueeze(1) * prev_le
            ...
            # h_t = torch.tanh(self.W_h(torch.cat((h_t_tilde_, curr_le), dim=1)))
            # 试试加法
            # h_t = self.W_h(h_t_tilde_+curr_le)
            # 试试拼接
            h_t = self.W_h(torch.cat((h_t_tilde_, curr_le), dim=1))
            # h_t_ = self.dropout(h_t)

            # h_t, c_t = self.lstmCell(current_interaction, prev_h, prev_c, curr_le)
            # h_list.append(h_t)
            h_list.append(h_t)
            c_list.append(c_t)
        # h0 is deleted
